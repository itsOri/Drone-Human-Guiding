...
DRONE_START_HEIGHT = 200 # 200 is good for outside
...
def get_ids_in_frame(results):
    ...
    human_ids = [int(box.id.item()) for box in results[0].boxes if hasattr(box, 'id') and box.id is not None and int(box.cls) == 0]
...
def get_target_id_in_frame(results, target_ids):
    ...
    print("ids_in_frame", ids_in_frame)
...
def find_id_in_frame(results, selected_id_container, persons_dict, all_persons_in_frame):
    ...
    if coords:
        ...
        print(f"[INFO] Counter value: {selected_id_container["lost_counter"]}")
        is_id_found = True
    else:
...
def get_coords(container):
    ...
    if coords and container["lost_counter"] > 0:
        coords = None
...
def main():
    ...
    selected_id_container = {"id": [], "history": []}
    selected_target_container = {"id": [], "history": []}
