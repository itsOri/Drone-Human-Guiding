...
def check_and_start_id_input(selected_id_container, id_selected, input_thread_running):
    """
    Starts a thread for ID input if not running,
    and returns updated (id_selected, input_thread_running) values.
    """
    if not id_selected and not input_thread_running:
        thread = threading.Thread(target=input_thread, args=(selected_id_container,))
        thread.daemon = True
        thread.start()
        input_thread_running = True

    if selected_id_container["id"] is not None and not id_selected:
        id_selected = True
        print(f"[INFO] Selected ID: {selected_id_container['id']}")
        # Place logic here to do something with selected_id_container["id"]

    return id_selected, input_thread_running
...
def process_selected_id(results, selected_id_container, id_selected, input_thread_running):
    """
    ...
    Returns updated (id_selected, input_thread_running).
    """
    if id_selected:
...
def main():
	...
	try:
		while True:
			...
			id_selected, input_thread_running = check_and_start_id_input(selected_id_container, id_selected, input_thread_running)
			
			id_selected, input_thread_running = process_selected_id(results, selected_id_container, id_selected, input_thread_running)
